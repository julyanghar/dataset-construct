...
def get_similarity(model, image_folder, image_file, retrieved_image_file, **args):
    ...
    elif args["similarity_type"] == "last_hidden_state":
...
def process_dataset(model, dataset, image_folder, **args):
    new_data = []
    for idx, data in tqdm(enumerate(dataset), total=len(dataset), desc="Adding img_similarity"):
        data = dict(data)
        data["img_similarity"] = get_similarity(model, 
                                        image_folder, 
                                        data["image"], 
